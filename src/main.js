...
/* ============================================================
 * 🔐 새로고침/탭 종료 시 자동 로그아웃 — 제거 (2026-06-18, PR #85)
 * ------------------------------------------------------------
 *   이전 의도: "다른 브라우저/앱에서 접속 시 로그아웃 상태로 시작"
 *   문제: 이 의도는 persistence(local) 와 무관하게 beforeunload 에서 signOut 하는
 *     방식으로 달성하면 안 됨. 새로고침 시마다 firebase:authUser 토큰이 삭제돼
 *     "한 번 로그인하면 유지" 가 깨졌음.
 *   진단 9회 끝에 확정된 원인 (PR #76, #79, #80, #82, #83, #84 가 모두 이 코드를
 *     못 잡고 다른 곳을 수정하느라 우회 시도만 함).
 *   해당 의도가 진짜 필요하면 별도 패턴 (예: sessionStorage 기반 마커 + 새 탭
 *     검출 등) 으로 구현해야 함. 이 PR 의 범위 아님.
 * ============================================================ */
// window.addEventListener('beforeunload', () => { ... auth.signOut() ... })  // ← 제거
